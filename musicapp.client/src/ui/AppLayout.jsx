...
function AppLayout() {
  ...
  const isCreateAlbumFormActive = useSelector(
    (state) => state.album.isCreateAlbumFormActive,
  );
  const isAddMusicFormActive = useSelector(
    (state) => state.album.isAddMusicFormActive,
  );
  ...
  return (
    <>
      {isCreateAlbumFormActive && <NewAlbumForm />}
      {isAddMusicFormActive && <AddMusicForm />}
      {copied && <Copied />}
      {(isLoadingMusic || isLoadingAlbum || isLoadingSettings) && <Loader />}
      <div className="h-screen flex flex-col relative text-color">
        <ToolBar />
