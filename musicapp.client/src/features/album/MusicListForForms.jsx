...
function MusicListForForms({
  ...
  const musicList = filter
    ? allMusic.filter((song) => !activeAlbum.musicList.includes(song.id))
    : filteredMusic
      ? filteredMusic
      : allMusic;
  ...
  return (
    <>
      {musicList.length > 0 ? (
        <ul
          className={`flex flex-col gap-1 ${className} no-scrollbar background-color rounded-md`}
        >
          {musicList.map((song) => {
            return (
              <Song
        ...
        <p className="text-xl text-center content-center h-[56vh]">
          {translation.AllMusicAlreadyInAlbum}
        </p>
      )}
    </>
  );
