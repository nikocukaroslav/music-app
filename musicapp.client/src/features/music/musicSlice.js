...
export const removeMusic = createAsyncThunk(
  ...
  async (id, { dispatch, getState }) => {
    ...
    if (activeAlbum) dispatch(removeFromAlbum(id));
    else dispatch(fetchMusic());
  },
);
...
const musicSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromSelected(state, action) {
      state.selectedMusic = state.selectedMusic.filter(
    ...
    playNextSong(state) {
      if (state.music.length === 0) return;
      const currentIndex = state.music.findIndex(
        (song) => song.id === state.musicId,
      );
    ...
    playPreviousSong(state) {
      ...
      const previousIndex =
        (currentIndex - 1 + state.music.length) % state.music.length;
